...
var Validate = function() {
    ...
    var POP_VALID_VERSIONS = ['1.0', '1.1', '2'];
    var POPv1_VALID_HASHTYPES = ['SHA-256'];
    var POPv2_VALID_HASHTYPES = ['SHA224', 'SHA256', 'SHA384', 'SHA512', 'SHA3-224', 'SHA3-256', 'SHA3-384', 'SHA3-512', 'OpList'];
    var POPv2_VALID_ANCHORTYPES = ['BTCOpReturn', 'ETHData', 'BTCBlockHeader'];
        ...
        async.waterfall([
                ...
                function(proofObject, wfCallback) {
                    ...
                    if (receiptHeader) { // header section was found, so this could be a pre-v2 receipt
                        receiptVersion = receiptHeader.pop_version;
                        ...
                        if (receiptType) {
                            var typeRegex = /^Thinkproof.*v2$/;
                            var isValidType = typeRegex.test(receiptType); // validate 'type' attribute value
                            if (isValidType) receiptVersion = '2';
                            if (!receiptVersion) return wfCallback('Invalid Thinkproof type - ' + receiptType);
                        }
                    }
                    if (!receiptVersion) return wfCallback('Cannot identify Thinkproof version');


                    // Ensure specified version is a valid Thinkproof version value
                    if (_.indexOf(POP_VALID_VERSIONS, receiptVersion.toString()) == -1) return wfCallback('Invalid Thinkproof version - ' + receiptVersion);
    ...
    function _validate1xReceipt(receipt, confirmAnchor, callback) {
        ...
        if (_.indexOf(POPv1_VALID_HASHTYPES, hashType) == -1) return callback('Invalid hash type - ' + hashType);
    ...
    function _validate2xReceipt(receipt, confirmAnchor, callback) {
        ...
        var hashTypeRegex = /^Thinkproof(.*)v2$/;
        var hashType = hashTypeRegex.exec(receiptType)[1];
        if (!hashType) return callback('Invalid Thinkproof type - ' + receiptType);

        if (_.indexOf(POPv2_VALID_HASHTYPES, hashType) == -1) return callback('Invalid Thinkproof type - ' + receiptType);
        ...
        async.forEachSeries(operations, function(operation, operationCallback) {
            if (_.has(operation, 'left')) {
                ...
                for (var x = 0; x < operation.anchors.length; x++) {
                    ...
                    if (_.indexOf(POPv2_VALID_ANCHORTYPES, anchorType) == -1) return operationCallback('Invalid anchor type - ' + anchorType);
    ...
    function _validateAnchors(anchors, hashTestRegex, receipt, callback) {
        ...
        for (var x = 0; x < anchors.length; x++) {
            ...
            if (_.indexOf(POPv2_VALID_ANCHORTYPES, anchorType) == -1) return callback('Invalid anchor type - ' + anchorType);
