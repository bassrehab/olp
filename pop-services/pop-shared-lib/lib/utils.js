 ...
 * to a proof_of_presence v3 ops array
 *
 ...
 */
function formatAsproof_of_presenceV3Ops (proof, op) {
  let proof_of_presenceV3Ops = proof.reduce((result, item) => {
    if (item.left) {
      item = { l: item.left }
    } else {
      item = { r: item.right }
...
module.exports = {
  ...
  isHex: isHex,
  isIP: isIP,
  formatAsproof_of_presenceV3Ops: formatAsPROOF_OF_PRESENCEV3Ops
}
