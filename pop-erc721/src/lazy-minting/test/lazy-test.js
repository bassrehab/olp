...
async function deploy() {
  ...
  let factory = await ethers.getContractFactory("LazyPNTNFT", minter)
...
describe("LazyPNTNFT", function() {
  it("Should deploy", async function() {
    const signers = await ethers.getSigners();
    const minter = signers[0].address;

    const LazyPNTNFT = await ethers.getContractFactory("LazyPNTNFT");
    const LazyPNTNFT = await LazyPNTNFT.deploy(minter);
    await LazyPNTNFT.deployed();
  });
