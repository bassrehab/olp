...
async function mintNFT({contract, ownerAddress, provider, gasPrice, setStatus, image, name, description}) {
  ...
  setStatus(`Minted token #${tokenId}. Please retain this ID#, you would need it for retrieval of your Proof-of-Presence NFT.`);
  return tokenId;
}
...
export default function Minter({
  ...
  const uploadView = (
    <div>
      <div style={{ margin: "0 0 20px 0" }}>Drop an image file or click below to select.</div>
      <Upload
        name="avatar"
  ...
  const nameField = (
    <Input style={{ margin: "10px 0 10px 0" }} placeholder="Enter a name for your image NFT" onChange={e => {
      setName(e.target.value);
    }}/>
  );

  const descriptionField = (
    <Input.TextArea style={{ margin: "10px 0 0 0" }} placeholder="Enter a description for your image" onChange={e => {
  ...
  const mintButton = (
    <Button style={{ margin: "20px 50px 0 0" }} type="primary" disabled={!mintEnabled} onClick={startMinting}>
